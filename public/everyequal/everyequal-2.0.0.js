(function (root, factory) {
  ...
  function everyMapEqual(
    ...
    const sourceEntries = Array.from(source.entries());
    const matchedSourceKeys = new Set();

    for (const [targetKey, targetValue] of target.entries()) {
      let matched = false;

      for (let i = 0; i < sourceEntries.length; i++) {
        if (matchedSourceKeys.has(i)) {
          continue;
        }
        const [sourceKey, sourceValue] = sourceEntries[i];
        ...
        if (keysEqual) {
          if (
            ignore &&
            ignore(targetKey, targetValue, sourceValue, depthIndex)
          ) {
            matched = true;
            matchedSourceKeys.add(i);
            break;
          }
