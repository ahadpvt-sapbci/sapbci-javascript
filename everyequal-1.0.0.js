(function (root, factory) {
  ...
  function everyEqual(
    ...
    if (result || "object" !== typeof target) {
      return result;
  ...
  function everyMapEqual(
    ...
    const maxKeys = Array.from(new Set(targetKeys.concat(sourceKeys)));
    for (const key of maxKeys) {
      ...
      if (ignore && ignore(key, targetValue, sourceValue, depthIndex)) {
        ...
        continue;
      } else if (target.has(key) && !source.has(key)) {
        return false;
      } else if (!target.has(key) && source.has(key)) {
        return false;
      } else if (seen.has(targetValue)) {
  ...
  function everyObjectEqual(
    ...
    for (const key of maxKeys) {
      ...
      if (ignore && ignore(key, targetValue, sourceValue, depthIndex)) {
        ...
        Settings["[object Object]"].checkDescriptor
      ) {
